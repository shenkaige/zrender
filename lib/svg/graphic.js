import { adjustTextY, getIdURL, getMatrixStr, getPathPrecision, getShadowKey, getSRTTransformString, hasShadow, isAroundZero, isGradient, isImagePattern, isLinearGradient, isPattern, isRadialGradient, normalizeColor, round4, TEXT_ALIGN_TO_ANCHOR } from './helper';
...
import { createVNode, vNodeToString, META_DATA_PREFIX } from './core';
...
function setTransform(attrs, m, compress) {
    if (m && !(noTranslate(m) && noRotateScale(m))) {
        ...
        attrs.transform = noRotateScale(m)
            ? "translate(".concat(round(m[4] * mul) / mul, " ").concat(round(m[5] * mul) / mul, ")") : getMatrixStr(m);
    }
}
...
export function brushSVGTSpan(el, scope) {
    ...
    if (hasSeparateFont(style)) {
        ...
        separatedFontStr += "font-size:".concat(fontSize, ";font-family:").concat(fontFamily, ";");
        if (fontStyle && fontStyle !== 'normal') {
            separatedFontStr += "font-style:".concat(fontStyle, ";");
        }
        if (fontWeight && fontWeight !== 'normal') {
            separatedFontStr += "font-weight:".concat(fontWeight, ";");
        }
        attrs.style = separatedFontStr;
    }
    else {
        attrs.style = "font: ".concat(font);
    }
    if (text.match(/\s/)) {
...
export function setClipPath(clipPath, attrs, scope) {
    ...
    var pathVersion = clipPath.path.getVersion();
    var preAttrs = (_a = defs[clipPathId]) === null || _a === void 0 ? void 0 : _a.attrs;
    if (!clipPathId) {
    ...
    else if (pathVersion != (preAttrs === null || preAttrs === void 0 ? void 0 : preAttrs.pathVersion)) {
        preAttrs.pathVersion = pathVersion;
        defs[clipPathId] = createVNode('clipPath', clipPathId, preAttrs, [brushSVGPath(clipPath, scope)]);
    }
